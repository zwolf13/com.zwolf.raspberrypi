...
function downloadVideos() {
    ...
    do
        ...
        TM_ID=`echo "${URL}" | awk -F '/' '{print $5}'`;
        sudo python3 /usr/bin/youtube-dl --format 'best[ext=mp4]/best' --output "${OUTPUT_FOLDER}/tokyomotion/%(title)s - ${TM_ID}.%(ext)s" --no-overwrites --restrict-filenames --recode-video mp4 --write-info-json --write-thumbnail ${cookies_parameter} "${URL}"
